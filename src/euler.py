...
:uT:        =  numpy.where( :phi: > 0.0, :u:, :uA:)
:pT:        =  numpy.where( :phi: > 0.0, :p:, :pA:)
:rhoB:        =  numpy.where( :phi: > 0.0, :rho:, :rhoA:)
:rhoT:        =  numpy.where( abs(:phi:/:dx:*2.0) > 1.0, :rhoB:, .5*(:phi:/:dx:*2.0 + 1.0)*(:rho:-:rhoA:) + (:rhoA:)   )
"""
ss.EOM(eom)
...
ss.variables['Et'].data =  ss.gfilter(numpy.where( x < numpy.pi , 1.0 / .4,1.0 / .4))
...
ss.variables['EtA'].data =  ss.gfilter(numpy.where( x < numpy.pi , 1.0 / .4,1.0 / .4))
ss.variables['rhoA'].data = ss.gfilter(numpy.where( x < numpy.pi , 10.0, 10.0))
...
while tt > time:
    ...
    if viz:
        ...
        if ss.PyMPI.master and (cnt%20 == 0) and True:
            plt.figure(2)
            plt.clf()
